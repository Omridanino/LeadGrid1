...
export const HeroFuturistic = ({
  ...
  const displayTitle = content?.headline || title || formData?.businessName || "Build Your Dreams";
  const displaySubtitle = content?.subheadline || subtitle || "AI-powered creativity for the next generation.";
  ...
  return (
    <div className={`h-screen relative overflow-hidden ${getBackgroundClasses(content?.backgroundStyle || 'default')}`}>
      <div className="h-screen uppercase items-center w-full absolute z-60 pointer-events-none px-10 flex justify-center flex-col">
        {badgeText && (
          <div className="mb-6 flex justify-center">
            <span className={`inline-block px-4 py-2 rounded-full text-sm font-medium ${getBadgeStyleClasses(content?.badgeStyle || 'glass-dark')} ${getTextStyleClasses(content?.badgeTextStyle || 'default')}`}>
              {badgeText}
            </span>
          </div>
        )}
        ...
        <div className="text-3xl md:text-5xl xl:text-6xl 2xl:text-7xl font-extrabold">
          <div className="flex space-x-2 lg:space-x-6 overflow-hidden">
            {titleWords.map((word, index) => (
              <div
                key={index}
                className={`transition-all duration-1000 ${index < visibleWords ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} ${getTextStyleClasses(content?.headlineStyle || 'white-text')}`}
                style={{ 
        ...
        <div className="text-xs md:text-xl xl:text-2xl 2xl:text-3xl mt-2 overflow-hidden font-bold">
          <div
            className={`transition-all duration-1000 ${subtitleVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'} ${getTextStyleClasses(content?.subheadlineStyle || 'white-text')}`}
            style={{ 
        ...
        {content?.description && (
          <div className="text-sm md:text-lg xl:text-xl 2xl:text-2xl mt-4 overflow-hidden">
            <div className={`transition-all duration-1000 ${subtitleVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'} ${getTextStyleClasses(content?.descriptionStyle || 'white-text')}`}>
              {content.description}
            </div>
          </div>
        )}
