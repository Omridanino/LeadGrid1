...
export const HeroSectionLamp = ({ formData, currentColors, content }: HeroSectionLampProps) => {
    ...
    return (
        <div className={`relative flex min-h-screen flex-col items-center justify-center overflow-hidden ${getBackgroundClasses(content?.backgroundStyle || 'default')} w-full rounded-md z-0`} dir="rtl">
            ...
            <div className="relative z-50 flex -translate-y-80 flex-col items-center px-5 text-center">
                {badgeText && (
                    <div className="mb-6 flex justify-center">
                        <span className={`inline-block px-4 py-2 rounded-full text-sm font-medium ${getBadgeStyleClasses(content?.badgeStyle || 'white-on-black')} ${getTextStyleClasses(content?.badgeTextStyle || 'default')}`}>
                            {badgeText}
                        </span>
                    </div>
                )}
                
                <h1 className={`mt-8 py-4 text-center text-4xl font-medium tracking-tight md:text-7xl ${getTextStyleClasses(content?.headlineStyle || 'gradient-blue-text')}`}>
                    {businessStory}
                </h1>
                ...
                {content?.description && (
                    <p className={`mt-6 max-w-2xl text-base leading-relaxed ${getTextStyleClasses(content?.descriptionStyle || 'white-text')}`}>
                        {content.description}
                    </p>
                )}
                    ...
                    {content?.buttons && content.buttons.length > 0 ? (
                        content.buttons.map((button: any, index: number) => (
                            button.visible !== false && (
                                <button
                                    key={index}
                                    className={`px-6 py-3 rounded-lg text-lg font-medium transition-all duration-300 ${getButtonStyleClasses(button.style || 'gradient-blue-ocean')} ${button.textStyle && button.textStyle !== 'default' ? getTextStyleClasses(button.textStyle) : ''}`}
                                >
