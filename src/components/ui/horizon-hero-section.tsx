...
export const HorizonHeroSection = ({ 
  ...
  return (
    <div ref={containerRef} className={`relative w-screen h-screen overflow-hidden bg-black ${className}`}>
      <canvas ref={canvasRef} className="absolute inset-0" />
      
      {/* Side menu */}
      <div ref={menuRef} className="fixed left-4 top-1/2 transform -translate-y-1/2 z-10">
        <div className="flex flex-col items-center space-y-4">
          <div className="w-8 h-8 border border-white/30 rounded-full flex items-center justify-center">
            <div className="w-4 h-4 bg-white/50 rounded-full"></div>
          </div>
          <div className="text-white text-sm writing-mode-vertical transform rotate-180">
            SPACE
          </div>
        </div>
      </div>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white z-10">
        <h1 ref={titleRef} className="text-6xl md:text-8xl font-bold mb-8 tracking-widest">
          {title}
        </h1>
        
        <div ref={subtitleRef} className="space-y-2 text-lg md:text-xl max-w-2xl">
          <p className="opacity-80">{subtitle1}</p>
          <p className="opacity-80">{subtitle2}</p>
        </div>
      </div>

      {/* Scroll progress indicator */}
      <div ref={scrollProgressRef} className="fixed bottom-8 right-8 flex items-center space-x-4 text-white text-sm z-10">
        <div>SCROLL</div>
